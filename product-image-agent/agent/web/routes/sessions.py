...
def register_session_routes(
    ...
    def api_session_messages(sid):
        ...
        # english_text + english_textgenerationtext（textfrontendenglish_textimagetext）
    ...
    def api_session_blackboard(sid):
        """english_textstatus（text UI english_text）"""
        engine = sessions.get(sid)
        if engine:
            return jsonify({
                "session_id": sid,
    ...
    def api_session_report(sid):
        """english_textreport（text/text/english_text）"""
        engine = sessions.get(sid)
        if engine:
    ...
    def html_session_report(sid):
        ...
        <title>textreport - {{ payload.title }}</title>
        <style>
        ...
        <div class='card topbar'><div><h1>{{ payload.title }}</h1><div class='muted'>Session: {{ payload.session_id }}</div></div><a class='btn' href='/' target='_self'>english_text</a></div>
        <div class='card'><h2>text</h2><div>{{ payload.summary.preference_summary }}</div></div>
        <div class='card'><h2>text</h2>{% for p in payload.plan_history %}<div class='tag'>v{{ p.plan_version }}</div><div class='muted'>{{ p.next_action or p.goal or p.intent }}</div>{% endfor %}</div>
        <div class='card'><h2>text</h2><ul>{% for r in payload.reflection_summary %}<li>{{ r }}</li>{% endfor %}</ul></div>
        <div class='card'><h2>textmessage</h2>{% for m in payload.messages %}<pre><strong>{{ m.role }}</strong>: {{ m.content }}</pre>{% endfor %}</div>
        </body></html>
        """

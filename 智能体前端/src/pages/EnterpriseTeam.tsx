...
export default function EnterpriseTeam() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      addToast(error instanceof Error ? error.message : '智能体团队状态读取失败', 'error');
      setTeam(null);
  ...
  const launch = async () => {
    ...
    if (selected.length === 0) { addToast('至少选择一个可用专业智能体', 'error'); return; }
    setLaunching(true);
    try {
      const result = await enterpriseTeamApi.launch({ goal: goal.trim(), workspaceId: workspaceId || undefined, specialistIds: selected });
      setRun(result.run);
      addToast(`首席运营智能体已创建任务 ${result.run.id}`, 'success');
    } catch (error) { addToast(error instanceof Error ? error.message : '企业目标启动失败', 'error'); }
    finally { setLaunching(false); }
  };

  return <div className="p-0">
    <div className="mb-6"><h1 className="text-2xl font-bold text-gray-900">智能体运营团队</h1><p className="mt-1 text-gray-500">首席运营智能体拆解企业目标，协调专业智能体、技能、工具接入（MCP）和数据连接器</p></div>
    {loading && <div className="rounded-lg border border-gray-200 bg-white py-20 text-center text-sm text-gray-500">正在读取真实智能体能力...</div>}
    {!loading && !team && <div className="rounded-lg border border-red-200 bg-red-50 py-20 text-center text-sm text-red-700">团队能力读取失败，禁止使用虚假智能体。</div>}
    {team && <>
      <section className="mb-6 grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-start gap-4"><div className="grid h-12 w-12 place-items-center rounded-lg bg-gradient-to-br from-blue-600 to-purple-600 text-white"><BrainCircuit className="h-6 w-6" /></div><div><h2 className="font-bold text-gray-900">首席运营智能体企业目标</h2><p className="mt-1 text-sm text-gray-500">真实创建规划智能体任务，不直接执行外部平台写入。</p></div></div>
          ...
          <textarea value={goal} onChange={(event) => setGoal(event.target.value)} rows={5} placeholder="例如：分析当前 Ozon 店铺商品，找出三款可优化商品，生成调研和商品刊登草稿，所有外部写入等待我确认。" className="w-full resize-none rounded-md border border-gray-300 px-4 py-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-gray-500">已选择 {selectedCount} 个专业智能体</span><button onClick={() => void launch()} disabled={launching} className="flex items-center justify-center gap-2 rounded-md bg-blue-600 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-60">{launching ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}启动首席运营编排</button></div>
        </div>
        <aside className="rounded-lg border border-blue-100 bg-blue-50 p-5"><div className="mb-4 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-blue-600"/><h2 className="font-bold text-gray-900">执行护栏</h2></div><div className="space-y-3 text-sm text-gray-700"><p>只读分析可自动执行</p><p>报告和本地草稿可自动创建</p><p>发布、改价、库存、广告、退款必须人工确认</p><p>未接入数据连接器的智能体禁止选择</p></div><div className="mt-5 rounded-md bg-white px-3 py-3 text-xs text-gray-600">已连接店铺通道：{team.operationSafety.connectedStoreChannels}<br/>外部写入适配器：{team.operationSafety.externalWriteAdapterConnected ? '已连接受控通道' : '未连接'}</div>{run && <div className={`mt-4 rounded-md border p-3 ${run.status === 'FAILED' ? 'border-red-200 bg-red-50' : run.status === 'COMPLETED' ? 'border-green-200 bg-green-50' : 'border-blue-200 bg-white'}`}><div className={`flex items-center gap-2 text-sm font-medium ${run.status === 'FAILED' ? 'text-red-800' : run.status === 'COMPLETED' ? 'text-green-800' : 'text-blue-800'}`}>{run.status === 'FAILED' ? <XCircle className="h-4 w-4"/> : run.status === 'COMPLETED' ? <CheckCircle2 className="h-4 w-4"/> : <LoaderCircle className="h-4 w-4 animate-spin"/>}{run.status === 'FAILED' ? '任务执行失败' : run.status === 'COMPLETED' ? '任务已完成' : '任务执行中'}</div><div className="mt-2 break-all text-xs text-gray-700">任务编号：{run.id} · {executionStatusLabel(run.status)}</div>{run.errorMessage && <p className="mt-2 text-xs leading-5 text-red-700">{customerApiErrorMessage(run.errorMessage, 500)}</p>}<button onClick={() => navigate('/agent-roadmap')} className="mt-3 text-xs font-medium text-blue-600">查看智能体状态</button></div>}</aside>
      </section>
      <div className="mb-4 flex items-center gap-2"><Users className="h-5 w-5 text-blue-600"/><h2 className="font-bold text-gray-900">专业智能体</h2></div>

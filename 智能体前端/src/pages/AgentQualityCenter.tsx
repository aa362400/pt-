...
const agentTypes: Array<{ value: AgentType; label: string }> = [
  { value: "PRODUCT_RESEARCHER", label: "选品智能体" },
  { value: "LISTING_OPTIMIZER", label: "刊登智能体" },
  { value: "IMAGE_CREATIVE", label: "图片智能体" },
  { value: "CUSTOMER_INSIGHT", label: "客户洞察智能体" },
  { value: "PROFIT_ANALYST", label: "利润智能体" },
  { value: "KEYWORD_EXPLORER", label: "关键词智能体" },
  { value: "CONTENT_WRITER", label: "内容智能体" },
  { value: "PLANNER", label: "规划智能体" },
  { value: "GENERAL_ASSISTANT", label: "通用智能体" },
];
...
export default function AgentQualityCenter() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "智能体质量数据加载失败");
    } finally {
      setLoading(false);
    }
  }, [agentType]);
  ...
  async function submitCorrection() {
    if (!correction.runId.trim() || !correction.field.trim() || correction.reason.trim().length < 10) {
      setError("人工纠错必须填写真实任务编号、修改字段和不少于 10 个字的原因。");
      return;
    }
    setBusy(true);
    try {
  ...
  async function submitPromptAction() {
    if (!promptAction || promptReason.trim().length < 10) {
      setError("提示词状态变更必须填写不少于 10 个字的人工复核原因。");
      return;
    }
    setBusy(true);
    try {
      ...
      setError(cause instanceof Error ? cause.message : "提示词状态变更失败");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1540px] space-y-5 px-5 py-6 lg:px-8">
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="flex items-center gap-2 text-blue-600"><Activity size={18} /><span className="text-sm font-medium">智能体数据闭环</span></div>
          <h1 className="mt-2 text-2xl font-semibold text-slate-950">智能体质量中心</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricTile label="样本量" value={latest ? String(latest.sampleSize) : "0"} detail="所选时间窗内真实智能体任务" />
      ...
      <div className={`flex items-start gap-3 rounded-md border px-4 py-3 ${evidence.tone}`}>
        ...
        <div className="text-sm">
          ...
          <p className="mt-1 opacity-90">{scores?.status === "COMPLETE" ? "反馈和路由归因覆盖已达到 95%，可用于版本比较。" : scores?.status === "PARTIAL" ? "当前分数仅供观察，不能据此自动切换模型或提示词。" : "没有可计算的真实样本，系统不会用 0 分冒充质量结果。"}</p>
        </div>
      </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-white">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="font-semibold text-slate-950">归因证据</h2><p className="mt-1 text-xs text-slate-500">反馈和路由必须都能关联到真实任务</p></div><GitBranch size={19} className="text-blue-600" /></div>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-5 p-5 text-sm">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-white"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 className="font-semibold text-slate-950">最近路由决策</h2><Route size={18} className="text-blue-600" /></div><div className="divide-y divide-slate-100">{routerDecisions.length ? routerDecisions.slice(0, 10).map((item) => <div key={item.id} className="px-5 py-3"><div className="flex items-center justify-between gap-3"><p className="font-medium text-slate-900">{item.selectedModel}</p><time className="text-xs text-slate-500">{dateTime(item.createdAt)}</time></div><p className="mt-1 truncate text-xs text-slate-500" title={item.runId}>任务：{item.runId} · 耗时：{item.latencyMs == null ? "未返回" : `${item.latencyMs} 毫秒`} · 质量：{item.qualityScore == null ? "未返回" : `${item.qualityScore} 分`}</p></div>) : <p className="px-5 py-8 text-center text-sm text-slate-500">暂无模型路由证据。</p>}</div></div>
      </section>
      ...
      <Modal open={correctionOpen} onClose={() => setCorrectionOpen(false)} title="登记人工纠错" width="max-w-2xl"><div className="space-y-4"><p className="rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-800">只有能够关联到真实任务的纠错才进入正式评估，填写不存在或其他组织的编号会被后端拒绝。</p><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm text-slate-700">任务编号 *<input value={correction.runId} onChange={(event) => setCorrection((current) => ({ ...current, runId: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">刊登编号（可选）<input value={correction.listingId} onChange={(event) => setCorrection((current) => ({ ...current, listingId: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700 sm:col-span-2">修改字段 *<input value={correction.field} onChange={(event) => setCorrection((current) => ({ ...current, field: event.target.value }))} placeholder="例如：商品标题" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">修改前<input value={correction.before} onChange={(event) => setCorrection((current) => ({ ...current, before: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">修改后<input value={correction.after} onChange={(event) => setCorrection((current) => ({ ...current, after: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label></div><label className="block text-sm text-slate-700">纠错原因 *<textarea value={correction.reason} onChange={(event) => setCorrection((current) => ({ ...current, reason: event.target.value }))} rows={4} placeholder="说明智能体错在哪里，以及人工为什么这样修改。" className="mt-1 w-full resize-none rounded-md border border-slate-300 px-3 py-2" /></label><div className="flex justify-end gap-2"><button type="button" onClick={() => setCorrectionOpen(false)} className="rounded-md border border-slate-300 px-4 py-2 text-sm">取消</button><button type="button" disabled={busy} onClick={() => void submitCorrection()} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">保存纠错证据</button></div></div></Modal>

      <Modal open={Boolean(promptAction)} onClose={() => setPromptAction(null)} title="确认提示词状态变更"><div className="space-y-4"><p className="text-sm text-slate-700">版本 <strong>{promptAction?.version}</strong> 将变更为“{promptAction ? promptStatusLabels[promptAction.status] : ""}”。灰度版本最多获得 5% 流量，切换主版本必须由人工确认。</p><label className="block text-sm text-slate-700">复核原因 *<textarea value={promptReason} onChange={(event) => setPromptReason(event.target.value)} rows={4} className="mt-1 w-full resize-none rounded-md border border-slate-300 px-3 py-2" /></label><div className="flex justify-end gap-2"><button type="button" onClick={() => setPromptAction(null)} className="rounded-md border border-slate-300 px-4 py-2 text-sm">取消</button><button type="button" disabled={busy} onClick={() => void submitPromptAction()} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">确认变更</button></div></div></Modal>
    </div>
  );
}

...
} from '../utils/data-analysis-presentation';

function money(value: number, currency = 'RUB') {
  return `${value.toLocaleString('zh-CN', { maximumFractionDigits: 2 })} ${dataAnalysisCurrencyLabel(currency)}`;
}
...
export default function DataAnalysisV2() {
  ...
  const cards = [
    { label: '利润测算样本', value: String(profit?.calculationCount ?? 0), note: dataAnalysisSourceLabel(profit?.source), icon: DollarSign },
    { label: '估算利润合计', value: profit ? money(profit.totalEstimatedProfit) : '未返回', note: '仅来自真实利润测算', icon: TrendingUp },
    { label: '同步商品样本', value: String(hotProducts.length), note: dataAnalysisSourceLabel(products?.source), icon: Package },

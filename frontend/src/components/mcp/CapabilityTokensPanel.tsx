...
export function CapabilityTokensPanel({
  ...
  const [description, setDescription] = useState('localagentenglish_text');
      ...
      <div className="grid gap-5 px-5 py-5 xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.9fr)]">
        <div>
          <div className="mb-4 flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-semibold text-[#1A1A2E]">agentenglish_text</h2>
              <p className="mt-0.5 text-xs text-[#8B93B5]">
                english_text、english_text、english_textyesenglish_textpublic Agent text
              </p>
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <label className="block">
              <span className="mb-1 block text-xs text-[#4A5578]">english_text</span>
              <select
                value={action}
            ...
            <label className="block">
              <span className="mb-1 block text-xs text-[#4A5578]">yestext（text）</span>
              <input
                type="number"
                min={1}
                max={60}
            ...
            <label className="block">
              <span className="mb-1 block text-xs text-[#4A5578]">english_text ID（text）</span>
              <input
                value={workspaceId}
            ...
            <label className="block">
              <span className="mb-1 block text-xs text-[#4A5578]">text</span>
              <input
                value={description}
                maxLength={200}
          ...
          {issued ? (
            <div className="mt-4 border-l-4 border-amber-400 bg-amber-50 px-4 py-3">
              <p className="text-xs font-semibold text-amber-900">english_text，english_text</p>
        ...
        <div className="min-w-0">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-xs font-semibold text-[#4A5578]">english_text</h3>
            <button onClick={() => void refresh()} className="text-xs text-[#6C63FF]">text</button>
          </div>
          {loading ? (
            <div className="flex h-28 items-center justify-center"><Loader2 size={16} className="animate-spin text-[#6C63FF]" /></div>
          ) : items.length === 0 ? (
            <div className="flex h-28 items-center justify-center border border-dashed border-[#DDE1F2] text-xs text-[#8B93B5]">english_text</div>
          ) : (
                  ...
                  <div key={item.id} className="flex items-start gap-3 border-b border-[#EEF0F7] px-1 py-2.5 last:border-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-[11px] text-[#8B93B5]">
                        {inactive ? (item.revokedAt ? 'english_text' : 'english_text') : `text ${new Date(item.expiresAt).toLocaleString('zh-CN', { hour12: false })}`}
                      </p>
                    </div>
                    {!inactive ? (
                      <button
